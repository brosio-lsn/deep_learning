...
Usage (example):
  python -m src.training.blackboard_denoising --setting local --denoise-rate 0.15
"""
...
from src.models.positional_encodings import (RelativePositionBias2D, SinusoidalPositionalEncoding, AbsolutePositionalEncoding2D)
...
def main():
    ...
    train_ds = BlackboardAdditionDenoisingStepDataset(train_problems, setting=args.setting, denoise_rate=args.denoise_rate)
    val_ds = BlackboardAdditionDenoisingStepDataset(val_problems, setting=args.setting, denoise_rate=args.denoise_rate, seed=args.seed + 1)
    ...
    for pe_name, pe in pes:
        ...
        print(f"\n===== Direction2 | {args.setting} | denoise={args.denoise_rate} | {pe_name} | {cfg_name} =====")
